...
public class JiraClient {
	 ...
	 *            Specifies which issue fields will be included in the result.
	 *            <br>
	 ...
	 * 
	 * @param expand
	 *            issue fields to expand when getting issue data
	 *
	 * @return an issue instance
	 *
	 * @throws JiraException
	 ...
	 *
	 * @param expandFields
	 *            Specifies with issue fields should be expanded
	 *
	 * @return a search result structure with results
	 *
	 * @throws JiraException
	 ...
	 * 
	 * @param maxResults
	 *            if non-<code>null</code>, defines the maximum number of
	 *            results that can be returned
	 * 
	 * @param startAt
	 *            if non-<code>null</code>, defines the first issue to return
	 *
	 * @return a search result structure with results
	 *
	 * @throws JiraException
	 ...
	 *             when the search fails
	 */
	public Issue.SearchResult searchIssues(String jql, String includedFields, String expandFields, Integer maxResults, Integer startAt)
			throws JiraException {
	...
	public List<CustomFieldOption> getCustomFieldAllowedValues(String field, String project, String issueType) throws JiraException {
		...
		List<CustomFieldOption> customFieldOptions = Field.getResourceArray(CustomFieldOption.class, fieldMetadata.get("allowedValues"),
				restclient);
	...
	public User getUser(String username) throws JiraException {
		try {
			...
			URI uri = restclient.buildURI(Resource.getBaseUri() + "user", params);
			JSON response = restclient.get(uri);
			return new User(restclient, (JSONObject) response);
		} catch (Exception ex) {
			throw new JiraException(ex.getMessage(), ex);
		}
	}
	 ...
	 *             failed to obtain the project
	 */
	public Role getProjectRole(String projectKey, String roleId) throws JiraException {
		try {
			URI uri = restclient.buildURI(Resource.getBaseUri() + "project/" + projectKey + "/role/" + roleId);
			JSON response = restclient.get(uri);
			return new Role(restclient, (JSONObject) response);
		} catch (Exception ex) {
			throw new JiraException(ex.getMessage(), ex);
		}
	}
	 ...
	 *             failed to obtain the project
	 */
	public Role getProjectRole(String url) throws JiraException {
		try {
			JSON response = restclient.get(new URI(url));
			return new Role(restclient, (JSONObject) response);
		} catch (Exception ex) {
			throw new JiraException(ex.getMessage(), ex);
		}
	 ...
	 * @throws JiraException
	 *             failed to obtain the issue type list.
	 */
	 ...
	 * 
	 * @throws JiraException
	 *             failed to obtain the component
	 */
